...
- Ubuntu 24.04 Noble 在 `kernel.apparmor_restrict_unprivileged_userns=1` 且 AppArmor enabled 时，本 Release 的 controller/`init` 明确不支持，必须在任何账号、unit、plugin、sudoers、policy 或其他持久 mutation 前 fail closed；不能把 `host-policy install`、direct bwrap smoke 或旧 managed state 当作解锁路径。`join`/PVE endpoint 不运行 Source Plugin，不受此 controller 限制，也不得触碰 host policy。Raw `host-policy`/`init`/`join` 仍必须固定明确的同一个 `OPS_AGENT_VERSION=vX.Y.Z` 并核对 payload version，tar/deb launcher 必须绑定相邻 versioned release-root。离线 tar 只能由 root 解入独占 staging；从用户/Agent 可写目录 sudo 执行、root path-chain/release-tree owner 或 DAC 不安全必须 fail closed。所有 native package 前置只能由模型外管理员在 Phase 1 前单独安装；release installer 不得调用包管理器，缺失命令必须 fail closed。AppArmor/bwrap 不能成为 server-only deb 的强依赖。
...
- workload 只能声明、组合或暴露已注册 capability；源码仅可在固定、无网络 bubblewrap host 中运行，并经有界 IPC 调用 scope-gated typed provider。业务 tool capability 与 provider name 是两套独立命名：descriptor capability 必须与获批 manifest 精确相等，但它不能代替 manifest `requestedScopes` 授权 provider；Core 只按 provider name 查找固定 policy，并逐项校验该 policy 的 required scopes。Workload host 的 outer/inner 两层都只可创建 `user/ipc/pid/net/mnt`，且两层都必须使用 `--proc /proc` 让 procfs 与各自 PID namespace 一致；outer 若只继承 service proc 视图，fixed inner 会在解析 namespace FD 时以 `/proc/<pid>/ns/ns` 不存在而失败。`ops-agentd.service` 的 `RestrictNamespaces=` 必须精确同步该最小集合，同时保留 `ProtectHostname=yes`、`ProtectProc=invisible`、`ProcSubset=all`、inner nested-userns deny 与仅供这两层固定 bwrap 建立/封闭 user namespace 使用的 `/proc/sys/user/max_user_namespaces` 窄可写 mount；安装探针必须在唯一、root-owned、位于 `/run/systemd/system` 的短生命周期 static unit 中复制并验证等价 hardening 与同一双层结构，结束后精确清理该 unit、drop-in、driver 与 nonce，runtime 还必须验证 outer sync FD + exact init process identity completion barrier。`ProcSubset=all` 是访问该 namespaced sysctl 的必要契约；`ProtectProc=invisible` 只隐藏其他 UID 的 PID 目录，不能被描述为隐藏 same-UID 进程或只读的非 PID procfs 全局元数据。除精确 sysctl 例外外必须保留 `ProtectKernelTunables=yes`，并同时保留 `PrivateDevices=yes`。每次 invoke 必须为 exact active digest 持有 registry shared invocation lease，register/activate/deactivate 必须取得同一 plugin lock 的 exclusive lease，禁止只靠 provider 前后重复读取 `current` 处理更新竞态。`adapter.tui` 与 `workload.base` 是必需的源码插件；真实 bubblewrap/user namespace 不可用时初始化必须在持久 mutation 前 fail closed。不得通过 agentd 内 `eval`/动态 import、宿主 shell fallback 或任意子进程把插件源码变成 core/root 逃逸路径；需要 root 的能力必须仍由版本化 broker tagged union 表达。
- Noble restricted-userns controller 的 hosted 证据已经闭合为“不支持”：outer 使用自身 `--proc /proc` 时在 `ProtectProc=invisible` 下返回 `EPERM`；只移除 outer proc remount 的候选又在 fixed inner 启动时返回 `bwrap: open /proc/3/ns/ns failed: No such file or directory`。因此本 Release 必须保留正确的双层 procfs/PID namespace contract，并在 `init` 持久 mutation 前拒绝该 host，而不是调整 sysctl、启用 SUID bwrap、改成单层、降低 `ProtectProc`/systemd hardening、使用 unconfined profile 或扩大 AppArmor authority。真正支持该组合需要独立、typed、短生命周期 spawn supervisor；在实现并通过同一 hosted gate 前不得宣称 production 支持。
- 旧 Release 可能已经留下 exact managed AppArmor files 或 loaded kernel profiles；它们是需保留的主机证据，不是当前 controller 的 support signal。默认卸载不得移除，本 Release 的 helper `remove` 必须始终 fail closed；`host-policy inspect/install` 都必须立即返回 unsupported，不能作为安装建议或恢复步骤；`status` 只做 strict exact legacy inventory：永不返回 `0`，`3` 仅表示 safely absent，`1` 表示 managed、drift 或 inaccessible，且 drift/inaccessible 可在状态正文前失败，任何结果都不能成为成功 gate。任何移除只能由另行设计和审计、能处理 active-label 竞态的主机维护流程完成。`join`/PVE endpoint 不安装 agentd 或 Source Plugin，永远不得安装、更新或删除这份 host policy。
...
- outer/inner 的 procfs contract 必须与双层 PID namespace contract 一起保持：outer 继续创建 PID namespace、保留默认 PID 1 reaper 与 sync/info/exact-identity completion evidence，并使用 `--proc /proc` 建立与 outer PID namespace 一致的 procfs；inner 也必须使用 `--proc /proc` 建立自己的私有 procfs。不能以继承 service proc 视图取代 outer mount，否则 fixed inner 无法可靠解析其 namespace FD；也不能删除任一 PID namespace、reaper 或 lease-settlement barrier。
- GitHub-hosted Noble run `31319405888`（commit `7091ecfbc28ae6410f06d4e2b64462c96dd83726`，job `93259846767`）已推翻 outer-proc-removal 候选：fixed inner 返回 `bwrap: open /proc/3/ns/ns failed: No such file or directory`。结合 outer `--proc /proc` 在同一 restricted-userns/`ProtectProc=invisible` static boundary 下的 `EPERM`，本 Release 对该 controller host 明确 unsupported；这不是继续扩大 profile 或削弱 sandbox 的依据。
