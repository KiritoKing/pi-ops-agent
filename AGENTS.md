...
- adapter 与 workload 都是不可信源码插件，不因第一方身份获得额外信任。每次安装或更新必须重新计算内容摘要、展示所请求 scope 并由用户在模型外本地流程逐次审批；授权必须同时绑定插件 ID、kind、摘要和 scope，摘要变化后旧授权立即失效。`plugin.register`、`plugin.install` 和 `workload.deploy` 永不允许 standing authorization。可执行 Source Adapter 与 Workload runtime 必须由 bubblewrap PID namespace 收拢进程树，源码入口作为 PID 1，禁止 detached/unref 后代越过摘要 lease 生命周期；Adapter 保留业务所需网络/宿主用户权限不等于它获得了额外信任。声明式 `adapter.tui` 不执行插件源码，compiled Client 必须自己持有 exact adapter digest lease 到退出，不能只依赖外层 runner 存活。
