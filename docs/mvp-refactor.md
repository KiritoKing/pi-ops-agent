...
| Release | static Go、Node runtime、tar/deb/SBOM/checksum/attestation、`._*` cleanup；disposable native ARM64 Linux 已完成 `0.3.0` tar/deb build 与 `verify-release.sh`；exact tested commit 的 GitHub-hosted PR CI（含真实 Linux Adapter runtime gate）已通过 | Release candidate/tag workflow 尚未运行；`ubuntu-22.04` hosted image 只是将退役前的过渡 gate，仍需迁移到后续受支持环境 |
...
| 场景 | 已取得的证据 | 仍不能证明 |
|---|---|---|
| Native ARM64 Release | disposable ARM64 Linux builder 对包含 endpoint broker readiness 修正的 `0.3.0` 候选成功构建 tar/deb；完整 `verify-release.sh` 通过 ELF、archive/deb payload parity、随包 Node `22.23.2` 与 Client/Reviewer 加载检查；该 readiness 修正已进入下述 exact tested commit | 这份本地 payload 不是由该 exact tested commit 的 Release workflow 生成、签署或发布；PR CI 成功不等于 amd64/arm64 Release candidate/tag workflow 已运行 |
...
| Non-PVE endpoint `join` | fresh non-PVE endpoint 完成严格 server/core-only join；真实 TLS 1.3 mTLS 验证 agent/observer role 分离，agent 经 HTTPS→core broker 完成 `host.snapshot`，observer prepare 与 agent approve 均被拒绝；重启 server/core broker 后再次读取成功且 endpoint healthcheck 为 0 failures / 0 warnings | 该轮 live join 暴露的 core socket readiness race 是本轮修正来源；修正虽已进入下述 exact tested commit 并通过 hosted installer gate，但该拓扑尚未用该 commit 的 artifact 重新跑 live smoke，也不包含模型或 Source runtime |
...
| Hosted PR CI | exact commit `0e90d4185eb0000f08d19431fc808f109ee08a32` 的 GitHub Actions [run 31326500051](https://github.com/KiritoKing/pi-ops-agent/actions/runs/31326500051) 全部成功：check job `93277617942`、installer-runtime job `93277617924`、adapter-linux-runtime job `93277617971`、join-installer-runtime job `93278084735` | 这是 PR CI，不是 Release candidate/tag workflow；没有生成或签署待发布的双架构 candidate，也没有创建 tag 或 Release |
| Hosted Adapter runtime | 同一 run 的 [adapter-linux-runtime job 93277617971](https://github.com/KiritoKing/pi-ops-agent/actions/runs/31326500051/job/93277617971) 在 `ubuntu-22.04` 上使用 root-owned、versioned Node `22.23.2` fixture，以及由 SHA-256 固定的 release source 构建、root-owned `0755`、无 SUID/SGID/file capability 的 bubblewrap `0.9.0`，成功运行未削弱的 `npm run test:adapter-linux-runtime`：保留 outer/inner 双层 PID/procfs、outer sync/info FD 与 exact init process identity completion barrier | 只证明该 hosted 环境中的真实 Adapter runtime probe；GitHub-hosted `ubuntu-22.04` image 已有[退役计划](https://github.com/actions/runner-images/issues/14254)，这里只能作为过渡 gate。它不证明 Noble restricted-userns controller、真实模型 Session、真实 Proxmox、BotMux 生产链或 Release candidate/tag；这些证据齐备前不发布 `v0.3.0` tag |
