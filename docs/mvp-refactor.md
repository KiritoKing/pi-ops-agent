...
表中的 hosted candidate 行是对应 source SHA 的历史证据；本轮 installer 修正与已完成本地/live
诊断复验的 `change.status` receipt race 修正，只有进入新的 exact candidate 并重新验证后，才能
继承同等 Release 结论：

| 场景 | 已取得的证据 | 仍不能证明 |
|---|---|---|
| Native ARM64 Release | disposable ARM64 Linux builder 对包含 endpoint broker readiness 修正的 `0.3.0` 候选成功构建 tar/deb；完整 `verify-release.sh` 通过 ELF、archive/deb payload parity、随包 Node `22.23.2` 与 Client/Reviewer 加载检查；该修正也已进入下述 hosted candidate 的 branch commit 与 synthetic source SHA | 这份本地 payload 与 hosted aggregate candidate 是两份独立证据；两者都未由 exact version-tag publish path 签署或发布 |
| Bookworm controller `init`/start | fresh OrbStack Debian 12 / systemd 252 controller 先执行失败路径并清点安装事务回滚，随后以修正后的同一候选成功完成 `init`、启动 controller topology，并得到 healthcheck `0 failures / 1 warning`；唯一 warning 是明确标识的 OrbStack 宿主基线提示，不是 managed unit、socket、credential、Plugin 或 broker failure | 这是 OrbStack/LXC 上 systemd 252 的 controller 安装、回滚与启动证据，不等于 bare-metal/普通 VM、真实模型 Session、PASSWD/TTY 审批或长期运行验证 |
...
| Fast async `change.status` receipt race | fresh fake-PVE E2E 暴露了 status snapshot 为 `EXECUTING`、签名前 durable state 已推进到 `COMMITTED` 时的 signed-receipt observation race；工作树已改为签名同一次校验得到的 immutable Store clone，不再二次读取可变 state，也不让 status 等待长 mutation lock。确定性并发 regression、Go race suite 与注入该 ARM64 broker 的 disposable live immediate-status 复验均通过：首个响应为 signed `EXECUTING`，fresh status 为 signed `COMMITTED` | live 复验使用的是从当前工作树交叉编译并注入既有 staging 的单一 broker binary，不是重新构建、验证和上传的完整 exact candidate artifact；新的 hosted candidate 通过前，不能把历史 aggregate candidate 的 Release 证据继承给这项修正 |
| Hosted Release candidate gates | branch commit `4683ececa1762eea6403d84901c286596ccf3442` 对应 PR synthetic merge/source SHA `03a9f07095108b28a8dea5a6fcd0a2d116f59ac4`；GitHub Actions Release [run 31327304520](https://github.com/KiritoKing/pi-ops-agent/actions/runs/31327304520) overall success：validate job `93279614816`、real Adapter job `93279833378`、amd64 build `93279833391`、arm64 build `93279833402`、reusable join job `93280123369` 与 candidate verification job `93280183652` 均成功 | 权威 candidate identity 是 PR synthetic source SHA，不是 branch SHA；该成功不等于 exact tag publish、attestation、真实模型 Session 或真实 Proxmox 验证 |
| Hosted Adapter runtime | 同一 Release run 的 [real Adapter job 93279833378](https://github.com/KiritoKing/pi-ops-agent/actions/runs/31327304520/job/93279833378) 在 `ubuntu-22.04` 上使用 root-owned、versioned Node `22.23.2` fixture，以及由 SHA-256 固定的 release source 构建、root-owned `0755`、无 SUID/SGID/file capability 的 bubblewrap `0.9.0`，成功运行未削弱的 `npm run test:adapter-linux-runtime`：保留 outer/inner 双层 PID/procfs、outer sync/info FD 与 exact init process identity completion barrier | 只证明该 hosted 环境中的真实 Adapter runtime probe；GitHub-hosted `ubuntu-22.04` image 已有[退役计划](https://github.com/actions/runner-images/issues/14254)，这里只能作为过渡 gate。它不证明 Noble restricted-userns controller、真实模型 Session、真实 Proxmox 或 BotMux 生产链 |
| Aggregate candidate / publish boundary | candidate verification 上传 artifact `9041956777`，名称 `release-candidate-03a9f07095108b28a8dea5a6fcd0a2d116f59ac4`，大小 `317346964` bytes；PR event 的 publish job `93280264262` 按契约 skipped，tag 与 release API 查询均返回 `404`，确认没有 `v0.3.0` tag 或 GitHub Release | 该 aggregate candidate 没有 provenance attestation，也未发布；真实模型 Session 与真实 Proxmox 仍未验证，这些边界闭合前不创建 `v0.3.0` tag |

上述 OrbStack/live E2E 尚未运行真实模型链，也未执行本地 PASSWD/真实 `/dev/tty` 二次确认，更未连接
真实 Proxmox。它们分别验证 installer/systemd、compiled HTTPS/mTLS client、typed broker、receipt、
restart/recovery 与 fake runner 边界，不能组合成对 model → Source Workload → Plugin lease → TTY → real
PVE 整条生产链的验证。
