...
| 场景 | 已取得的证据 | 仍不能证明 |
|---|---|---|
| Native ARM64 Release | disposable ARM64 Linux builder 对包含 endpoint broker readiness 修正的 `0.3.0` 候选成功构建 tar/deb；完整 `verify-release.sh` 通过 ELF、archive/deb payload parity、随包 Node `22.23.2` 与 Client/Reviewer 加载检查 | 该 readiness 工作树尚未推送，因而尚无其 exact commit 的 GitHub-hosted amd64/arm64 publish gate 结果；本地构建成功不等于 Release 已可签署 |
...
| Non-PVE endpoint `join` | fresh non-PVE endpoint 完成严格 server/core-only join；真实 TLS 1.3 mTLS 验证 agent/observer role 分离，agent 经 HTTPS→core broker 完成 `host.snapshot`，observer prepare 与 agent approve 均被拒绝；重启 server/core broker 后再次读取成功且 endpoint healthcheck 为 0 failures / 0 warnings | 该轮同版本 live join 暴露的 core socket readiness race 是本轮修正来源；非 PVE 证据来自修正前 exact HEAD，当前 readiness 候选在该拓扑上尚未重新取得 exact-artifact smoke，也不包含模型或 Source runtime |
...
| Hosted/production | 旧 exact HEAD `4312053e13cc9426e36cc1be01101df97b717b48` 的 GitHub Actions [run 31320718580](https://github.com/KiritoKing/pi-ops-agent/actions/runs/31320718580) 已全部通过；另有本地 TS/Go、readiness-patched native ARM64 verifier、Noble fail-closed、non-PVE/PVE endpoint smoke 与 fake-PVE recovery evidence | readiness diff 当前尚未推送、没有 exact-head CI；完整 Release publish gates、真实模型 Session、真实 Proxmox 环境仍未验证，BotMux 在 observed restricted-userns + AppArmor enabled host 的真实生产链仍不支持；这些证据齐备前不发布 `v0.3.0` tag |
