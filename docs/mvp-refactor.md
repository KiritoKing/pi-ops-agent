...
| Guardian | 同 UID heartbeat、PID/UID/exe/cgroup/starttime 复核、有界 TERM/KILL；旧 root helper 已退出 release/runtime，uninstaller 仍兼容清理；fresh Bookworm ARM64 已完成 exact identity-bound hang/restart、gateway 存活与同 Session reconnect smoke | 长期 soak 与非 OrbStack/bare-metal restart 验证 |
...
| Exact `c9cc4002` controller readiness / rollback E2E | branch HEAD `c9cc4002e98d7b026ce6265e93236864e2303588` 的 push CI [run 31499001148](https://github.com/KiritoKing/pi-ops-agent/actions/runs/31499001148)、PR CI [run 31499006148](https://github.com/KiritoKing/pi-ops-agent/actions/runs/31499006148) 与 PR Release [run 31499006410](https://github.com/KiritoKing/pi-ops-agent/actions/runs/31499006410) 均为 attempt 1 overall success；Release 的 `publish` 按 PR contract skipped。non-publishing synthetic merge/source SHA 为 `35945804d62f9436a5f56661e4e43c63b4e9333d`；aggregate artifact `9104419972` 名为 `release-candidate-35945804d62f9436a5f56661e4e43c63b4e9333d`，Actions API digest 为 `sha256:8b523cbdb79407b138ef259f7dbe1c0242471cfbb38f98d4305c2cc63fd034a2`；ARM64 native packages artifact `9104324738` 的 API digest 为 `sha256:28d887996c0cfd5685ff5a4f78c52537e361db10470e3db900411d29b25c65f4`，ARM64 SBOM artifact `9104329753` 的 API digest 为 `sha256:7a02af8da5aca0f4c0ee8d455398bc17a9d71b56e89146a227b2a55a87f4e09c`，aggregate 内部 ARM64 tar 为 `sha256:d6c3709f4e0e744badd6ee9accffb9d01ea30f31af808659e203e73f0d8138cf`，其中 installer 与 branch 文件 `sha256:0013635ab21f17f595081632aca44b5b6eea4b46267a278b4f52757a9cd447c2` 逐字相同。fresh isolated、无外网的 `opsagent-readiness-c9cc-20260811` OrbStack Debian 12 ARM64 先验证未授权必需 Plugin 的 `init` 以 rc `1` fail closed 并完整恢复 fresh inventory；随后只批准 exact `adapter.tui` digest `sha256:230fbaad8b45fda68ea715ecd16db9be419258a5fde4408d21a8bd46767da32a` 与 `workload.base` digest `sha256:534be6a85ffd44457e3c6c941b25c40ae241492626d28f5fd1c04d7dad5211e2`，在 `OPS_AGENT_TEST_FAIL_AT=services` 注入后以 rc `97` 完整回滚到 fresh inventory，再正式完成 `init --no-start`。测试只写入 VM 内部随机、非生产的 encrypted fixture credential，没有 prompt 或 model/API 流量。第一次从 heartbeat absent 启动成功；停止后保留 stale 旧 heartbeat 再次执行 `START_NOW` 也成功，锁定的新非零 agentd PID 与新 heartbeat inode 均不同于旧 generation，旧 PID 已消失。最终 healthcheck 为 `0 failures / 1 warning`，唯一 warning 仍是 OrbStack 基线 `sys-kernel-debug.mount`/`kmod-static-nodes.service`；agentd/gateway `NRestarts=0`，changes/requests/locks 均为 0，Agent audit 的 prompt/tool 事件为 0，全部 invocation lease 已释放 | 这是 exact `c9cc4002` synthetic candidate 在 fresh Bookworm/OrbStack 上对拒绝路径、transaction rollback、no-start install 与两代 heartbeat readiness 的实测，不是外部真实模型、真实 Proxmox、bare-metal/普通 VM、长期 soak 或 exact version-tag publish/attestation 证据；随机 fixture credential 没有离开 VM，也没有触发 provider 请求 |
| Earlier full-chain controller / scripted inspect E2E | branch commit `93ece25426bb4bec3bd78d1a1bae971a4b561a29` 的 PR CI [run 31350405965](https://github.com/KiritoKing/pi-ops-agent/actions/runs/31350405965) 与 PR Release [run 31350406119](https://github.com/KiritoKing/pi-ops-agent/actions/runs/31350406119) 均成功；non-publishing synthetic merge/source SHA 为 `5ee4603dfd4d6ba6495748c742f5a282c0837118`，aggregate artifact `9048809866` 的 Actions API digest 为 `sha256:f9caa8fc65fcba832f61a483e54e53b3a8d5e05e105fc3e4ca1f0ccfed36f02e`，内部 ARM64 tar 为 `sha256:626a2620777f80c881f5e8e15f5eff76017292a9a183bb981e157e00b340ee76`。fresh isolated `opsagent-final-head-93ece25-20260810` OrbStack Bookworm ARM64 / systemd 252 controller 从该 tar 完成 `init --no-start`，启动后 healthcheck 为 `0 failures / 1 warning`（仅上述 OrbStack baseline），gateway 首次启动 `NRestarts=0`；production TUI idle writer 使 exact Adapter lease 从 busy 恢复为 free。root-owned strict current fixture `sha256:0f93b54a84ba3f555ccb51a3fda52d658369df75bbb58fd5ac354c723417380b` 的 unit/effective verifier 通过。首次随机 credential 与 fixed-token fixture server 不一致是 harness 配置错误；production 链收到 `401` 后 fail closed，tool/change 均为 0，root state/audit 不变且 lease 全部释放。轮换为公开、非 secret fixture token 后，production compiled TUI → gateway → agentd → Pi `openai-completions` SSE → `workload.base` 双层 bubblewrap → `ops_inspect` → server/core root broker `host.snapshot` 全链成功并产生 `SCRIPTED_MODEL_E2E_INSPECT_OK`，fixture 精确收到 2 个请求；changes/requests/locks 均为 0，root state SHA 保持原值，root audit 只新增一条 inspect metadata，未记录输出正文，Workload 与 TUI lease 均释放。最后恢复原 config/credential，精确移除活动 runtime 的 fixture unit、安装文件与用户并确认 unit `LoadState=not-found`；root-only staging 输入保留作证据，最终 healthcheck 仍为 `0 failures / 1 warning` | 这是 deterministic scripted OpenAI-compatible transport，不是真实模型推理、外部 provider TLS/rate-limit/retry、真实 Proxmox 或 tag/publish 证据；首次 credential mismatch 是 fixture setup error，不是 production bug |
| Guardian / gateway restart live E2E | branch commit `6a03c35f686528efd831d30360446e3380ed6f50` 的 PR CI [run 31349106357](https://github.com/KiritoKing/pi-ops-agent/actions/runs/31349106357) 与 PR Release [run 31349106437](https://github.com/KiritoKing/pi-ops-agent/actions/runs/31349106437) 均成功；non-publishing synthetic merge/source SHA 为 `955a812e41c189e09231bbabb86c7f3302beaf45`，candidate artifact `9048424460` 的 Actions API digest 为 `sha256:9d6d5c9cbd363b95a3d2312097c7ba151e838af107045e569b22ede0e4a269bf`，内部 ARM64 tar 为 `sha256:f32fb8160d69e00c2175ad1ac0146b04a38b0ee2fb11dfe7320f581ec5dd842a`。fresh `opsagent-guardian-6a03c35-20260810` OrbStack Bookworm ARM64 / systemd 252 controller 首次启动全部 active，gateway `NRestarts=0`，连续 60 次以上 canonical heartbeat 未出现 timestamp 错误。真实 compiled TUI 以固定 Session ID 建立 idle writer，exclusive lease 被拒；随后只对复核过 identity 的 agentd PID `11137` 发送 `SIGSTOP`，guardian PID `11132` 将其终止，systemd 以 PID `11793` 恢复 agentd 且 `NRestarts=1`，gateway PID `11138` / `NRestarts=0` 与 guardian PID / `NRestarts=0` 均保持不变。旧 TUI 因 backend EOF 自动退出并释放 lease；backend 恢复后 fresh TUI 使用同一 Session ID 成功连接、重新持有 lease，再经 Ctrl-D 正常退出并释放。前后 state/Targets hashes 不变，root audit 保持原一行不变，Agent audit 只新增预期的 `agentd_started`，prompt/tool 事件均为 0；最终 healthcheck 为 `0 failures / 1 warning`，唯一 warning 仍是上述 OrbStack baseline | 这是绑定 exact PR synthetic candidate 的 guardian→agentd restart、gateway continuity、writer/admission 释放与 fresh reconnect 证据；它不是 tag/publish/attestation，不验证外部真实模型、真实 Proxmox、长期 soak 或非 OrbStack 主机 |
...
| PVE fake E2E | fresh `opsagent-e2e-pve-fullchain-20260810` strict fake-PVE endpoint（d0fb/`64eb83b` candidate）中，额外 `--force` argv 以 rc `2` 被拒且 fixture state SHA 前后相同；controller 上 exact compiled HTTPS client 经真实 mTLS 访问，并在 `change.status` 路径验证 pinned PVE receipt，初始只读 inspect 得到 guest `stopped`。automatic/standing change `pve-change-5a74b1eb961ee9f2d36f93e890778de0` 经 fresh signed status 从 `EXECUTING` 进入 `COMMITTED`；held change `pve-change-3d900898a163d7188d96d9f962ea72ae` 在仅重启 PVE broker 后沿同一 UPID 进入 `COMMITTED`，fixture `nextTask=2` 且只存在一个 terminal `OK` task，证明未重复 create。最后 failure change `pve-change-8a1c847c401dfd3e1385050da502dfc6` 得到签名 `RECOVERY_REQUIRED`，唯一 task 为 `stopped/ERROR`、`effectApplied=false`、guest 保持 `stopped`，`pve/vmid/100` lock 精确保留给该 change；endpoint health 仍为 `0 failures / 0 warnings` | Harness 从 compiled HTTPS client 开始，明确绕过 model、Source Workload host 与 Plugin invocation lease；fake fixture 不具备真实 Proxmox、pmxcfs、quorum、storage 或 guest side effect，因此不是 real PVE 验证；保锁 endpoint 只保留 recovery evidence，不再重置复用 |
| `workload.pve` Source composition regression | 测试从仓库真实 `plugins/workload-pve` tree 按 production snapshot-v1 规则重算并固定 `sha256:a791bb69fe620f25c87b7d91ea551ab43826d49ecc70aebbd9797b91234a0f20`，从 exact CAS path import `workload.mjs`，经 production descriptor/schema、`createSourceWorkloadTools`、`createTrustedBaseProviderCatalog` 的真实 PVE lifecycle binding、current digest revalidation 与 framed shared lease，到 production typed provider 的 `change.prepare`/fresh `change.status`。断言实际 operation 注入 exact Plugin ID/digest 与 policy/capability revision、prepare side-channel、lease 在 provider 调用期间持有并在 ACK 后释放、source audit 精确落盘；缺 capability、缺 requested scope、schema 错误与 invoke 前 current digest drift 均在触达 provider 前 fail closed | 这是 production composition 的可执行单元回归，host transport 使用 snapshot runner stub；它不替代 OrbStack 双层 bubblewrap、真实 TUI 本地审批、live Agent→Source→fake endpoint 或 real Proxmox 证据 |
| Live `workload.pve` approval boundary | controller 上 source inspection 仍精确得到上述 digest，但 `agentd-pluginctl current --plugin-id workload.pve` 为 rc `3`；旧 change `change-522decf46927d9d5c50cc23d38d4d116` 仍是未批准、无 authorization、无 lock 的 `PENDING_APPROVAL plugin.register`。原 Client 已退出，Client 内存中的 user-intent binding 已消失；相同 Session ID resume 按设计不恢复该 binding，因此旧 ref 的 `/approve` 必须 fail closed，不能用 direct root `agentd-pluginctl register --approved-by ...` 伪造批准 | live Agent→Source→fake-PVE 全链只能由本地用户先在 fresh TUI 经 `/status` 后以真实 PAM/TTY `/reject` 清理旧 pending，再启动 deterministic fixture，在同一 live TUI turn fresh prepare，并亲自完成两次 `/approve`、PAM 与 exact `/dev/tty` confirmation；Agent 不能代输这些授权 |
...
| Exact-head aggregate independent reverify | branch commit `864122ec0aeed8030b4c153e4d2360a97a44c2f8` 的 push CI [run 31352553415](https://github.com/KiritoKing/pi-ops-agent/actions/runs/31352553415)、PR CI [run 31352555970](https://github.com/KiritoKing/pi-ops-agent/actions/runs/31352555970) 与 PR Release [run 31352556050](https://github.com/KiritoKing/pi-ops-agent/actions/runs/31352556050) 全部成功；synthetic merge `a38a8e179c0b6ea2500b80953ddf15e0f591ea08` 的 aggregate artifact `9049535874` API digest `sha256:e7b569fd72a71c1bd6918f0cf88404abcc8ab0b4277f743609d5410f1ac5b835` 经 connector 独立下载后外层 ZIP SHA 精确相同、压缩结构完整、全部内层 `checksums.txt` 通过，manifest 精确绑定 version `0.3.0`、repository 与 synthetic SHA。ARM64 tar `sha256:4cd26ac4fc1e831f20c888894d1e082262f35cc1eb6fd0ef882153b5a561d5ab`、deb `sha256:a35e20e028cc6bab2853bb5c99d4663dff37f98815c858ce1315620747d22677` 逐字传入 disposable Bookworm ARM64 root-only staging；当前 verifier 的 non-executing 模式再次通过 numeric owner、mode、tar/deb parity、control/postinst、ELF 与 JavaScript syntax 检查，下载 payload 未被执行 | 这是 PR synthetic candidate 的独立 supply-chain/parity 复验，不是 exact tag publish、GitHub attestation、外部真实模型或 real Proxmox 证据 |
