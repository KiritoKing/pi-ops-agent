...
| Non-PVE endpoint `join` | fresh non-PVE endpoint 完成严格 server/core-only join；真实 TLS 1.3 mTLS 验证 agent/observer role 分离，agent 经 HTTPS→core broker 完成 `host.snapshot`，observer prepare 与 agent approve 均被拒绝；重启 server/core broker 后再次读取成功且 endpoint healthcheck 为 0 failures / 0 warnings | 该轮 live join 暴露的 core socket readiness race 是本轮修正来源；修正虽已进入下述 branch commit/synthetic source 并通过 hosted reusable join gate，但该拓扑尚未用 hosted aggregate candidate 重新跑 live smoke，也不包含模型或 Source runtime |
...
| PVE fake E2E | 同一 fresh strict fixture 的额外 argv 被拒且状态摘要不变，typed read 返回 guest stopped/unlocked；standing start 经 fresh signed status/receipt 验证进入 `COMMITTED`；held task 在 PVE broker restart 后沿同一 UPID 从 `EXECUTING` 续跑到 `COMMITTED` 且仅产生一个 task；最终 failure 得到签名 `RECOVERY_REQUIRED`，task 为 `stopped/ERROR`、guest 保持 `stopped`，cluster-global VMID lock 与恢复证据保留 | Harness 从 typed HTTPS client 开始，明确绕过 model、Workload Source host 与 Plugin invocation lease；fake 不具备真实 PVE cluster、pmxcfs、quorum、storage 或 guest side effect，因此不是实际 Proxmox 验证 |
