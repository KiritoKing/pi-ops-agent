...
bwrap 默认 PID 1 reaper、使用 `--proc /proc` 建立与 outer PID namespace 一致的 procfs，且只启动
固定 inner bwrap。inner 使用 `--proc /proc --as-pid-1 --disable-userns`，让
...
inner PID namespace 的私有 procfs；不能省略 outer proc mount，否则 fixed inner 无法解析其
namespace FD。bwrap 自己通过
